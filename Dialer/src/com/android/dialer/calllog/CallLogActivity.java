...
public class CallLogActivity extends TransactionSafeActivity implements ViewPager.OnPageChangeListener,
    ...
    protected void onResume() {
        ...
        ///M: fix ICE15-1517 at 2017/06/29 @{
        if (mIsSearchUi) { 
            mIsSearchUi = false;
            enterSearchUi();
            if (mSearchView != null) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (item.getItemId() == android.R.id.home) {
			///Add by fanta for ICE15-150 start
			if (mInSearchUi) {
            // We should let the user go back to usual screens with tabs.
            	exitSearchUi();
        	}else{///Add by fanta for ICE15-150 end
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        if (mInSearchUi) {
            outState.putBoolean("SearchUI", mInSearchUi);
            outState.putCharSequence("SearchText", mSearchView.getText());
        }
    }

    private boolean mIsSearchUi = false;
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            mIsSearchUi = savedInstanceState.getBoolean("SearchUI", false);
            if (mIsSearchUi && mSearchFragment == null) {
                addSearchFragment();
            }
            mSearchText = savedInstanceState.getCharSequence("SearchText");
        }
    }
    ///M: @}
}
