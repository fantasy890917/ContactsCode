...
public class CallLogQueryHandler extends NoNullCursorAsyncQueryHandler {
    ...
    public void markMissedCallsAsRead() {
        Log.i(TAG,"CallLogQueryHandler markMissedCallsAsRead:"+PermissionsUtil.hasPhonePermissions(mContext));
        if (!PermissionsUtil.hasPhonePermissions(mContext)) {
            return;
        }
    ...
    public void fetchMissedCallsUnreadCount() {
        Log.i(TAG,"CallLogQueryHandler fetchMissedCallsUnreadCount:"+PermissionsUtil.hasPhonePermissions(mContext));
        if (!PermissionsUtil.hasPhonePermissions(mContext)) {
            return;
        }
    ...
    private void updateMissedCallsUnreadCount(Cursor statusCursor) {
        final Listener listener = mListener.get();
        if (listener != null) {
            Log.i(TAG,"CallLogQueryHandler updateMissedCallsUnreadCount");
